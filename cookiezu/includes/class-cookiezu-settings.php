...
class CookiEzu_Settings {
    ...
    public static function save() {
        ...
        $sanitized = array(
            ...
            // v1.3.0
            'banner_language'          => in_array( $data['banner_language'] ?? '', array( 'en', 'ar', 'ms' ) ) ? $data['banner_language'] : 'en',
            'policy_version'           => sanitize_text_field( $data['policy_version'] ?? '1' ),
            'extended_disclosure'      => wp_kses_post( $data['extended_disclosure'] ?? '' ),
            'data_processing_location' => sanitize_text_field( $data['data_processing_location'] ?? '' ),
        );
